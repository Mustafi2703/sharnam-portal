  ...
  <div style="max-width:640px;margin:0 auto;padding:24px;">
    ...
    <div style="background:#fff;border:1px solid #e2e5eb;border-radius:12px;padding:24px;">
      ...
      <p style="margin:0 0 16px;color:#5c6578;font-size:14px;line-height:1.5;">
        Dear ${escapeHtml(opts.name)}, your login for <strong>${escapeHtml(opts.projectCode)}</strong> is ready.
      </p>
      ...
      <p style="margin:0;font-size:11px;color:#94a3b8;">Change the password after first sign-in.</p>
    </div>
  </div></body></html>`;
}
...
export async function sendProjectPortalInvites(
  ...
  const allow = new Set((memberIds || []).filter(Boolean));

  const members = await prisma.projectMember.findMany({
    where: { projectId, ...(allow.size ? { userId: { in: [...allow] } } : { id: { in: [] } }) },
...
export async function emailProjectSetupBrief(_opts: {
  ...
  return { skipped: true as const, sent: [] as string[], reason: "onboarding_brief_disabled" };
}
