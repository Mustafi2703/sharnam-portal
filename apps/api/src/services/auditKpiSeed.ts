...
export async function seedAuditKpiFromSheets(prisma: PrismaClient, projectId: string) {
  if (!fs.existsSync(AUDIT_PACK) || !fs.existsSync(KPI_PACK)) {
    console.warn("Audit/KPI packs missing — skip seed");
  ...
  const audit = await prisma.siteAudit.upsert({
    ...
    create: {
      ...
      projectId,
      title: "Site document audit — demo",
      auditRef: "AUD-2026-01",
      status: "InProgress",
      leadAuditor: "Document Controller",
      plannedDate: new Date(),
      scopeNotes: "Seeded from SITE_AUDIT_Pack.xlsx",
    },
    update: {},
  });

  const awb = XLSX.readFile(AUDIT_PACK);
  ...
  for (const [sheet, section] of [
    ...
    const items = parseChecklistSection(rows);
  ...
  const kwb = XLSX.readFile(KPI_PACK);
  ...
  if (subjectWs) {
    ...
    for (const row of rows) {
      ...
      await prisma.kpiSubject.upsert({
        ...
        update: { name, rag: cell(row[15]) || "UNUSED" },
      });
      subjectCount++;
    }
  }
