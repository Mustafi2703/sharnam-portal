...
export const PORTAL_LOGINS: Record<string, PortalConfig> = {
  ...
  office: {
    ...
    subtitle: "CRM, bid management, project modules, reports, and user access — full PMC control.",
    demoEmail: "office@sharnam.demo", allowedRoles: ["office", "admin", "employee"],
    points: ["CRM · bids · projects", "ISO DMS per project", "Roles & audit"],
  ...
  client: {
    ...
    subtitle: "Published GFC, progress, reports, and concerns — read-only on assigned projects. Bid BOQs are vendor-only.",
    demoEmail: "client@sharnam.demo", allowedRoles: ["client"],
    points: ["Published drawings", "Progress & WPR packs", "Raise concerns"],
    cta: "Sign in", tone: "#1E40AF", icon: "CL",
    landingPath: "/dashboard", workspaceKey: "progress", group: "role",
    policies: [...SHARNAM_PORTAL_POLICIES, "Read-only on published GFC unless raising a concern. No bid uploads."],
  },
  stakeholder: {
  ...
  hr: {
    ...
    headline: "HRMS desk",
    subtitle: "Recruitment → pre-joining → onboarding, employee files on _HR vault, attendance, leave, payroll, and letters. Not project ISO modules.",
    demoEmail: "anushka.jha@spdc.in",
    allowedRoles: ["admin", "office", "hr"],
    points: ["Recruit to onboard", "Employee files (_HR)", "Payroll & letters"],
    cta: "Sign in", tone: "#0D9488", icon: "HR",
    landingPath: "/hrm", workspaceKey: null, group: "role",
    policies: [...SHARNAM_PORTAL_POLICIES, "HRMS data is restricted to authorised HR staff. Project DMS stays per project."],
  },
};
